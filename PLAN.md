...
- ⏳ **[#125](https://github.com/mliang1604/pharos/issues/125)** Models clip away when zooming out (far plane too near, zoom unbounded) `[rendering]` ⚠ no milestone
  Raise the camera far plane (100 → 1000) and clamp `OrbitControls` `maxRadius` (200) so zoom can't recede the scene past the clip. _In review (`bug/125_ZoomFarClip`)._
